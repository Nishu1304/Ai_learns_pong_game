...
class Game:
    ...
    def __init__(self, window, window_width, window_height):
        ...
        self.left_paddle = Rodes(
            10, self.window_height // 2 - Rodes.HEIGHT // 2)
        self.right_paddle = Rodes(
            self.window_width - 10 - Rodes.WIDTH, self.window_height // 2 - Rodes.HEIGHT // 2)
    ...
    def _handle_collision(self):
        ...
        if ball.x_vel < 0:
            if left_paddle.y <= ball.y <= left_paddle.y + Rodes.HEIGHT:
                if ball.x - ball.RADIUS <= left_paddle.x + Rodes.WIDTH:
                    ball.x_vel *= -1

                    middle_y = left_paddle.y + Rodes.HEIGHT / 2
                    difference_in_y = middle_y - ball.y
                    reduction_factor = (Rodes.HEIGHT / 2) / ball.MAX_VEL
                    y_vel = difference_in_y / reduction_factor
                    ball.y_vel = -1 * y_vel
                    self.left_hits += 1

        else:
            if right_paddle.y <= ball.y <= right_paddle.y + Rodes.HEIGHT:
                if ball.x + ball.RADIUS >= right_paddle.x:
                    ball.x_vel *= -1

                    middle_y = right_paddle.y + Rodes.HEIGHT / 2
                    difference_in_y = middle_y - ball.y
                    reduction_factor = (Rodes.HEIGHT / 2) / ball.MAX_VEL
                    y_vel = difference_in_y / reduction_factor
                    ball.y_vel = -1 * y_vel
    ...
    def move_paddle(self, left=True, up=True):
        ...
        if left:
            if up and self.left_paddle.y - Rodes.VEL < 0:
                return False
            if not up and self.left_paddle.y + Rodes.HEIGHT > self.window_height:
                return False
            self.left_paddle.move(up)
        else:
            if up and self.right_paddle.y - Rodes.VEL < 0:
                return False
            if not up and self.right_paddle.y + Rodes.HEIGHT > self.window_height:
                return False
